...
\subsection{Datasets}
The datasets are generated with LuPNT~\cite{vila_lupnt_2025}, an open-source simulator for astrodynamics, communication, and Positioning, Navigation, and Timing (PNT). The simulator is written in C++ and has Python bindings. The lunar surface module uses Unreal Engine to generate stereo images, semantic labels, dense depth maps, and LiDAR point clouds. Terrain is based on lunar south pole Digital Elevation Models (DEMs) with procedural rocks and craters. The simulator supports multi-agent models and allows setting the Sun position and light sources.

We use two datasets for training and evaluation:

\begin{itemize}
	\item \textbf{Training set:} Contains 14,400 stereo image pairs with depth and semantic labels. Data were collected along spiral paths around rovers, landers, and astronauts. The set includes variations in camera height, Sun position, and lighting.
	\item \textbf{Evaluation set:} Contains 4,300 stereo image pairs from rover trajectories around a lander. These data include camera effects, low Sun elevation, and rover lights to test the models in conditions different from the training set.
\end{itemize}
